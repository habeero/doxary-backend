...
Phase 2.1 adds `operation_results` in migration `20260916_0002`. `result_payload` is PostgreSQL JSONB (portable SQLAlchemy JSON in other engines) but may only be written from a validated `AnalysisResult`; reads deserialize and revalidate it. `expires_at` is mandatory and reads treat expired or deleted records as unavailable. The cleanup worker remains deferred to a later Phase 2 increment; this phase provides deterministic expiry detection only.

Phase 2.2 adds `temporary_inputs` in migration `20260916_0003`. It persists only the server storage reference and safe metadata needed by a future worker; original bytes remain in the local temporary store. It is tied to `operation_id`, never to a server Document or `client_document_id` primary key.
