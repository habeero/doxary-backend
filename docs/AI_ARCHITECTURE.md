...
Provider failures retain only bounded technical diagnostics on the operation attempt (exception class, HTTP status, provider code/type/param, and request ID where available). SDK metadata is read only from allowlisted scalar fields, including the nested error metadata used by OpenAI status exceptions; provider messages, raw response bodies, request content, and credentials are never retained. The server injects Doxary's local `client_document_id` and fixed schema version after provider output and before domain validation, so server-owned identifiers are not part of the provider schema.
...
The provider transport schema narrows `amounts[].value` from Pydantic's
number-or-string `Decimal` representation to a JSON number. This prevents
locale-formatted or otherwise human-readable monetary text from reaching the
authoritative Decimal parser. The domain contract remains strict and does not
normalize currency symbols, grouping separators, qualifiers, ranges, or prose.
