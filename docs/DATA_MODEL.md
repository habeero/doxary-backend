...
| `operation_results` | Temporary delivery of one validated `AnalysisResult` for an operation: opaque result ID, operation ID, schema version, JSON/JSONB payload, created/expiry/deletion timestamps. | Foreign key to `operations`; unique operation ID prevents conflicting final results. Indexed expiry; no source files or full OCR. | Phase 2.1 |
| `temporary_inputs` | Temporary logical submission metadata: operation ID, `pdf`/`images` kind, server storage reference, ordered file metadata/digests, and lifecycle timestamps. | One-to-one with an operation; no bytes or raw document text. Indexed expiry and explicit deletion marker. | Phase 2.2 |
...
Phase 2.1 adds `operation_results` in migration `20260916_0002`. `result_payload` is PostgreSQL JSONB (portable SQLAlchemy JSON in other engines) but may only be written from a validated `AnalysisResult`; reads deserialize and revalidate it. `expires_at` is mandatory and reads treat expired or deleted records as unavailable. Terminal result delivery and temporary-input cleanup are implemented; broader scheduled retention/expiry operations remain an operational concern.

Phase 2.2 adds `temporary_inputs` in migration `20260916_0003`. It persists only the server storage reference and safe metadata consumed by the durable worker; original bytes remain in the local temporary store. It is tied to `operation_id`, never to a server Document or `client_document_id` primary key. Terminal processing records deletion in the lifecycle row and removes the temporary file.

Phase 2.5 reads `operations` and `operation_results` without changing them. Public delivery revalidates payloads as `AnalysisResult v1`; expiry/deletion is an explicit lifecycle outcome, not a successful null result.
